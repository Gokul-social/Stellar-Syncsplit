 ...
 * then has each call create_split on the SYNC_SPLIT Soroban contract.
 *
 * Each user gets a unique split description and amount.
 * Output: scripts/users_30.json + docs/user_feedback_30.csv
 *
 * Usage: node scripts/create_30_users.mjs
 */
...
const TEST_USERS = [
  ...
  { name: 'David Okonkwo',   email: 'david.okonkwo@gmail.com',   rating: 4, feedback: 'Really cool concept. The equal/proportional split modes saved me time.',   scenario: "Birthday Celebration — David's party", xlm: 30 },
  { name: 'Elena Volkov',    email: 'elena.volkov@gmail.com',    rating: 5, feedback: 'Love the real-time event feed. You can watch payments confirm live.',       scenario: "Vacation — Elena's group",           xlm: 18 },
  // Extended 25 users (Level 6)
  { name: 'Fatima Al-Rashid', email: 'fatima.alrashid@gmail.com', rating: 5, feedback: 'Incredible UX! Setting up a split for my flatmates was effortless.',       scenario: "Flat Rent Split — Fatima's place",  xlm: 100 },
  ...
  { name: 'Hana Yamamoto',   email: 'hana.yamamoto@gmail.com',   rating: 5, feedback: 'Love that it\'s on-chain. No trust issues in the group anymore!',           scenario: "Concert Tickets — Hana's friends",  xlm: 40 },
  { name: 'Ivan Kozlov',     email: 'ivan.kozlov@gmail.com',     rating: 4, feedback: 'Great for travel expenses. Calculating proportional shares is easy.',       scenario: "Euro Trip — Ivan's travel crew",    xlm: 80 },
  ...
  { name: 'Lena Müller',     email: 'lena.muller@gmail.com',     rating: 4, feedback: 'Really intuitive flow. The Freighter signing popup is seamless.',           scenario: "Ski Trip — Lena's club",            xlm: 60 },
  { name: 'Marco Rossi',     email: 'marco.rossi@gmail.com',     rating: 5, feedback: 'Using SYNC_SPLIT for our Italian family reunion. Works flawlessly!',       scenario: "Family Reunion — Marco's family",   xlm: 50 },
  ...
  { name: 'Omar Hassan',     email: 'omar.hassan@gmail.com',     rating: 4, feedback: 'Great project. Would love USDC support in next version.',                  scenario: "Sports Event — Omar's crew",        xlm: 28 },
  ...
  { name: 'Rafael Santos',   email: 'rafael.santos@gmail.com',   rating: 5, feedback: 'Stellar testnet is so fast. Transactions confirm in seconds.',             scenario: "Barbecue Party — Rafael's friends", xlm: 19 },
  ...
  { name: 'Wendy Park',      email: 'wendy.park@gmail.com',      rating: 4, feedback: 'Sharing the invite link made it so easy to add participants.',             scenario: "Baby Shower — Wendy's party",       xlm: 27 },
  ...
  { name: 'Yuki Nakamura',   email: 'yuki.nakamura@gmail.com',   rating: 5, feedback: 'Fast, secure, and transparent. Stellar blockchain makes it trustworthy.',  scenario: "Sushi Dinner — Yuki's group",       xlm: 24 },
  { name: 'Zara Ahmed',      email: 'zara.ahmed@gmail.com',      rating: 4, feedback: 'The wallet connection works with both Freighter and xBull. Impressive.',   scenario: "Eid Celebration — Zara's family",   xlm: 42 },
  { name: 'Amos Okafor',     email: 'amos.okafor@gmail.com',     rating: 5, feedback: 'Built on Soroban so the contract is verifiable. Love the transparency.',   scenario: "Lagos Meetup — Amos's group",       xlm: 20 },
  ...
  { name: 'Diana Kovacs',    email: 'diana.kovacs@gmail.com',    rating: 5, feedback: 'Real-time settlement is the killer feature. No more IOU drama!',           scenario: "Wedding Gift Fund — Diana's group", xlm: 70 },
];
...
async function invokeContract(server, method, params, keypair) {
  ...
  if (StellarSdk.SorobanRpc.Api.isSimulationError(sim)) throw new Error(`Sim: ${sim.error}`);
  ...
  if (result.status !== 'SUCCESS') throw new Error(`TX ended: ${result.status}`);
...
async function main() {
  ...
  for (let i = 0; i < TEST_USERS.length; i++) {
    ...
    catch (err) { console.error(`   ❌ Account: ${err.message}`); continue; }
  ...
  console.log(`✅ ${ok.length}/${TEST_USERS.length} users interacted with contract\n`);
  results.forEach((r, i) => {
    const s = r.createTxHash !== 'FAILED' ? '✅' : '❌';
    console.log(`${s} ${i + 1}. ${r.name} — ${r.publicKey}`);
  ...
  console.log('\n💾 Saved: scripts/users_30.json');

  // ─── Save CSV ─────────────────────────────────────────────────────────────
  const csvHeader = 'No,Name,Email,Rating,Feedback,Wallet Address,Transaction Hash,Split ID,Explorer';
  ...
  writeFileSync('./docs/user_feedback_30.csv', [csvHeader, ...csvRows].join('\n'));
  console.log('💾 Saved: docs/user_feedback_30.csv');

  console.log('\n🎉 Done! Import user_feedback_30.csv into Excel for submission.\n');
}
